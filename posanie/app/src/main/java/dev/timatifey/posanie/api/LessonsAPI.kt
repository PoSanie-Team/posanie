...
class LessonsAPI(private val dispatcher: CoroutineDispatcher) {
    ...
    suspend fun isWeekOddByGroupId(groupId: Long, date: String): Boolean =
        withContext(dispatcher) {
            val url = "$LESSONS_BASE_URL/scheduler/$groupId?date=$date"
    ...
    suspend fun isWeekOddByTeacherId(teacherId: Long, date: String): Boolean =
        withContext(dispatcher) {
            val url = "$LESSONS_BASE_URL/teachers/$teacherId/scheduler?date=$date"
            return@withContext url
                .getJsonObjectIgnoringContentType()
                .getJSONObject("week")
                .getBoolean("is_odd")
    ...
    companion object {
        private const val LESSONS_BASE_URL = "$BASE_URL/api/v1/ruz"
    }
}
...
private fun getLessonsBy(schedulerTypeId: Long, url: String): Map<WeekDay, List<Lesson>> {
    ...
    for (dayIndex in 0 until daysArray.length()) {
        ...
        val weekDay = WeekDay.getWorkDayByOrdinal(dayObj.getInt("weekday").adjust())
...
private fun JSONObject.toLesson(lessonId: Long): Lesson {
    val teacherFullName = if (getString("teachers") != "null") {
...
private fun Int.adjust() = this % 7 + 1

private fun String.getJsonObjectIgnoringContentType(): JSONObject = Jsoup
    .connect(this)
    .ignoreContentType(true)
    .execute()
    .body()
    .run {
        JSONObject(substring(indexOf("{")))
    }
